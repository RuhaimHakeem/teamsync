...
export function TaskDetail({
  ...
  const projectMembership = projectsQuery.data
    ?.find((project) => project.id === taskQuery.data?.projectId)
    ?.members.find((member) => member.userId === userQuery.data?.id);

  const canUpdateStatus = Boolean(
    userQuery.data &&
    taskQuery.data &&
    (userQuery.data.role === "ADMIN" ||
      userQuery.data.id === taskQuery.data.assigneeId ||
    ...
    <main className="min-h-screen bg-neutral-50 px-4 py-6 tablet:px-6">
      <div className="mx-auto max-w-4xl">
        <Link
          ...
          href={
            returnProjectId
              ? `/dashboard?projectId=${returnProjectId}`
              : "/dashboard"
          }
        >
          ← Back to dashboard
          ...
          <div className="mt-4 space-y-4">
            <section className="rounded-[8px] border border-neutral-200 bg-white p-6 shadow-sm">
                  ...
                  {canUpdateStatus ? (
                    <label>
                      ...
                      <select
                        ...
                        className="rounded-[6px] border border-primary bg-white px-2 py-1 text-[13px] font-normal text-primary outline-none focus:ring-2 focus:ring-primary/20"
              ...
              {statusMutation.isError ? (
                <p className="mt-3 rounded-[8px] border border-danger px-3 py-2 text-[13px] font-normal text-danger">
              ...
              <dl className="mt-6 grid gap-3 rounded-[8px] bg-neutral-50 p-4 text-[15px] font-normal tablet:grid-cols-2">
                <div>
                  <dt className="text-[13px] font-normal text-neutral-500">
                    Assignee
                  </dt>
            ...
            <section className="rounded-[8px] border border-neutral-200 bg-white p-6 shadow-sm">
              ...
              >
                ...
                {errors.body ? (
                  <p className="text-[13px] font-normal text-danger">
                    {errors.body.message}
                  </p>
                ) : null}
